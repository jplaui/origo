...
func ComputeWitness() (witness.Witness, error) {
	...
	ecb1Assign := u.StrToIntSlice(params["ecb1"], true)
	ecb0Assign := u.StrToIntSlice(params["ecb0"], true)
	// witness definition record
	ivAssign := u.StrToIntSlice(params["IV"], true)
	...
	// fmt.Println("len ecb1ByteLen:", ecb1ByteLen)
...
func readOracleParams() (map[string]string, error) {
	...
	// read in record params
	record_idx, _ := strconv.Atoi(record_pub["record_index"])
	recConf, err := u.ReadMMAtIdx("./local_storage/record_confirmed.json", record_idx)
	if err != nil {
		log.Error().Msg("u.ReadMMAtIdx")
		return nil, err
	}
	...
	// get iv for record index
	tags, err := u.ReadMMAtIdx("../client/local_storage/recordtag_public_input.json", record_idx)
	if err != nil {
		log.Error().Msg("u.ReadMMAtIdx")
		return nil, err
	}
