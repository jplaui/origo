...
func (p *Parser) StoreConfirmedKdcParameters() error {
	...
	err = os.WriteFile("./local_storage/kdc_confirmed.json", file, 0644)
	if err != nil {
...
func (p *Parser) CheckAuthTags(rps map[string]map[string]string) error {
	...
	// fmt.Println("record_tag_public_input:", authPI)
	...
	for seq, record := range rps {
		...
		// fmt.Println("trying sequence:", seq, "and record:", record)
		...
		if ok {
			...
			// compute authtag
			tag := AuthTag13(ecb1, cipherChunks, ecb0, ad)

			// verify authtag
			if tag != c {
				// fmt.Println("failing sequence:", seq)
		...
		// fmt.Println("++sequence increment")
	}
